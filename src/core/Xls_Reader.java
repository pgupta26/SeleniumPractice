...
public class Xls_Reader {
	...
	public Xls_Reader(String path) {
		...
		try {
			...
			/*
			 * fis = new FileInputStream(path); workbook = new
			 * XSSFWorkbook(fis); sheet = workbook.getSheetAt(0);
			 * 
			 * fis.close();
			 */
			...
			try {
				workbook = (XSSFWorkbook) WorkbookFactory.create(fpis);
			} finally {
				fpis.close();
			}
		} catch (Exception e) {
	...
	// color in the cell
	public boolean ColorColumn(String sheetName, String colName, int rowNum) {
		try {
			fis = new FileInputStream(path);
			workbook = new XSSFWorkbook(fis);
